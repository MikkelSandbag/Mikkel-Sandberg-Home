...
(function (factory) {
  ...
  */
   ...
   */
		addCursor: function addCursor() {
			if (this._cursor) {
				...
				this._cursor.stop().remove();
			}

			this._cursor = $('<span>|</span>').css(this.cursorStyles).insertAfter(this._container);
   ...
   */
		remove: function remove(length, callback) {
			if (length <= 0) {
				callback();
				this.timer = null;
				return this.fire('end_remove.typist').type();
			}
   ...
   */
		step: function step(text, callback) {
			...
			if (!text.length) {
				callback();
				this.timer = null;
				return this.fire('end_type.typist').type();
			}
   ...
   */
		type: function type() {
			...
			if (item.delay) {
				this.fire('start_pause.typist').timer = setTimeout($.proxy(function () {
					callback();
					this.timer = null;
					this.fire('end_pause.typist').type();
				}, this), item.delay);
				return;
			} else if (item.remove) {
				this.fire('start_remove.typist').remove(item.remove, callback);
				return;
			} else if (item.stop) {
				this.stop();
				return;
			...
			this.fire('start_type.typist').step(item.text, callback);
		}
	};
});